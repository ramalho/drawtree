...
from tree import class_tree, render_lines, main


def test_tree_1_level():
    result = list(class_tree(TabError))
    assert result == [(TabError, 0, True)]


def test_tree_2_levels():
    result = list(class_tree(IndentationError))
    assert result == [
...
def test_tree_from_type_metaclass():
    """
    The `type` class is a special case in the `class_tree` generator because
...
def test_render_lines_3_levels_1_leaf():
    ...
    class Y(X): pass
    class Z(Y): pass
    result = list(render_lines(class_tree(X)))
    expected = [
        'X',
        '└── Y',
        '    └── Z',
...
def test_render_lines_4_levels_1_leaf():
    ...
    result = list(render_lines(class_tree(Level0)))
    expected = [
        'Level0',
        '└── Level1',
        '    └── Level2',
...
def test_render_lines_2_levels_2_leaves():
    ...
    result = list(render_lines(class_tree(Branch)))
    expected = [
        'Branch',
        '├── Leaf1',
        '└── Leaf2',
...
def test_render_lines_3_levels_2_leaves_dedent():
    ...
    result = list(render_lines(class_tree(A)))
    expected = [
        'A',
        '├── B',
        '│   └── C',
